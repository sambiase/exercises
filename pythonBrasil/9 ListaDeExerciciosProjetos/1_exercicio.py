'''
    Controle de cotas de disco. A ACME Inc., uma organização com mais de 1500 funcionários, está tendo problemas
    de espaço em disco no seu servidor de arquivos. Para tentar resolver este problema, o Administrador de Rede
    precisa saber qual o espaço em disco ocupado pelas contas dos usuários, e identificar os usuários com maior
    espaço ocupado. Através de um aplicativo baixado da Internet, ele conseguiu gerar o seguinte arquivo, chamado
    usuarios.txt:
            ...
            Neste arquivo, o primeiro campo corresponde ao login do usuário e o segundo ao espaço em
            disco ocupado pelo seu diretório home. A partir deste arquivo, você deve criar um programa
...
with open('usuarios.txt', 'r') as arquivo_usuario:          #lê o arquivo usuarios.txt
    for linha in arquivo_usuario:
        ...
        lista_de_dados.append((bytes_consumidos, usuario))
...
Nr.  Usuário        Espaço utilizado     % do uso\n
'''
nr_usuarios_listados = int(input('Listar quantos usuarios? --> '))
lista_de_dados.sort(reverse=True)
lista_de_dados = lista_de_dados [:nr_usuarios_listados]

with open('relatorio.txt', 'w') as arquivo_relatorio:
    arquivo_relatorio.writelines(cabecalho)
    espaco_total_ocupado = sum([tamanho for tamanho, _ in lista_de_dados])  # soma os valores consumidos em MB
    media = espaco_total_ocupado / len(lista_de_dados)
    for indice, dados in enumerate(lista_de_dados):
        bytes_consumidos, usuario = dados                                   # desempacotar a lista
